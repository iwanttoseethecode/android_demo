...
public class ItemTouchHelper extends RecyclerView.ItemDecoration
    ...
    private void doChildClickEvent(MotionEvent event) {
        ...
        View consumEventView = mSelected.itemView;
        if (consumEventView instanceof ViewGroup){
            consumEventView = findConsumeView((ViewGroup) consumEventView, event.getRawX(),event.getRawY());
        }
        if (consumEventView != null){
            consumEventView.performClick();
        }
    }

    private View findConsumeView(ViewGroup consumEventView, float rawX, float rawY) {
        for (int i = 0; i < consumEventView.getChildCount(); i++){
            View child = consumEventView.getChildAt(i);
            if (child instanceof ViewGroup && child.getVisibility() == View.VISIBLE){
                ...
                if (isInBounds((int) rawX,(int) rawY,child)){
                    return child;
                }
            }
        }

        if (isInBounds((int) rawX,(int) rawY,consumEventView)){
            return consumEventView;
        }
        return null;
    }
